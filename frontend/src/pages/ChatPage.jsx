...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const chatId = selectedPartner.chatId || selectedPartner._id || selectedPartner.id
    sendTypingStatus(selectedPartner.id, true)
    if (sendActivityStatus) {
      sendActivityStatus(chatId, 'typing', true)
    }
    ...
    typingTimeoutRef.current = setTimeout(() => {
      ...
      if (sendActivityStatus) {
        sendActivityStatus(chatId, 'typing', false)
      }
    }, 2500)
  ...
  const startRecording = async () => {
    try {
      ...
      if (selectedPartner && sendActivityStatus) {
        const chatId = selectedPartner.chatId || selectedPartner._id || selectedPartner.id
        sendActivityStatus(chatId, 'recording_audio', true)
      }
  ...
  const stopRecording = (shouldSend = true) => {
    if (selectedPartner && sendActivityStatus) {
      const chatId = selectedPartner.chatId || selectedPartner._id || selectedPartner.id
      sendActivityStatus(chatId, 'recording_audio', false)
    }
  ...
  const startMediaUpload = async (file, localUrl, type, text, parentMessage = null) => {
    ...
    if (chatId && sendActivityStatus) {
      sendActivityStatus(chatId, 'uploading_file', true)
    }
    ...
    try {
      ...
      if (chatId && sendActivityStatus) {
        sendActivityStatus(chatId, 'uploading_file', false)
      }
    }
  }
}
...
return (
  <div className="flex h-full w-full bg-white dark:bg-gray-950 overflow-hidden font-sans">
            ...
            filteredPartners.map((partner) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3.5 p-3.5 rounded-3xl cursor-pointer transition-all duration-150 relative overflow-hidden ${isSelected
    ...
    <div className={`flex-1 flex flex-col h-full bg-bg-app overflow-hidden ${selectedPartner ? 'flex' : 'hidden md:flex'
      }`}>
            ...
            <div className="flex items-center gap-4 w-full">
              ...
              >
                ...
                <div className="text-left leading-tight flex-1 min-w-0">
                  ...
                  <p className="text-xs text-text-body opacity-60 truncate mt-0.5">
                    {(() => {
                      const chatId = selectedPartner.chatId || selectedPartner._id || selectedPartner.id
                      const chatActivities = userActivities[chatId] || {}
                      const activeUserIds = Object.keys(chatActivities).filter(id => id !== currentUser.id)

                      if (activeUserIds.length > 0) {
                        const activeUser = chatActivities[activeUserIds[0]]
                        const actText = activeUser.activity === 'recording_audio'
                          ? 'recording audio...'
                          : activeUser.activity === 'uploading_file'
                            ? 'uploading a file...'
                            : 'typing...'
                          ...
                          <span className="text-accent font-semibold animate-pulse">
                            {selectedPartner.isGroup
                              ? (activeUserIds.length > 1
                                ? `${activeUserIds.length} people active...`
                                : `${activeUser.username} is ${actText}`)
                              : actText}
                          </span>
                        )
                      }

                      if (typingStatus[selectedPartner.id] || typingStatus[chatId]) {
                        return <span className="text-accent font-semibold animate-pulse">typing...</span>
                      }

                      if (selectedPartner.isGroup) {
                        return selectedPartner.email || `${selectedPartner.members?.length || 0} members`
                      }

                      if (onlineUsers.includes(selectedPartner.id)) {
                        return 'Online'
                      }

                      if (selectedPartner.lastSeen) {
                        return `Last seen ${new Date(selectedPartner.lastSeen).toLocaleString()}`
                      }
              ...
              {selectedPartner.isGroup && (
                <button
                  ...
                  className={`p-2 rounded-xl text-gray-400 hover:text-accent hover:bg-accent/8 dark:hover:bg-accent/10 transition-all cursor-pointer ${isDetailsDrawerOpen ? 'text-accent bg-accent/8 dark:bg-accent/10' : ''
                    }`}
                  title="Group Details"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              ...
              messages.map((msg, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-start gap-2.5 max-w-[85%] ${isOwnMessage ? 'self-end justify-end ml-auto' : 'self-start justify-start mr-auto'
                      } ${isConsecutivePrev ? 'mt-0.5' : 'mt-3'} ${msg.reactions && msg.reactions.length > 0 ? 'mb-2.5' : ''
                      } animate-bubble`}
                  >
                    ...
                    <div
                      ...
                      {msg.isDeleted ? (
                        <p className={`text-left break-words select-none pb-3.5 italic flex items-center gap-1.5 ${isOwnMessage ? 'text-white/60' : 'text-gray-400 dark:text-gray-500'
                        ...
                        <>
                          ...
                          {msg.parentMessageId && (() => {
                            ...
                            return (
                              <div
                                ...
                                className={`mb-2.5 p-2 rounded-xl text-xs border-l-3 text-left cursor-pointer transition-all select-none hover:bg-black/8 dark:hover:bg-white/8 ${isOwnMessage
                          ...
                          {msg.fileAttachment && msg.fileAttachment.url && (
                            <div className="mb-2 max-w-full overflow-hidden rounded-xl border border-black/5 dark:border-white/5 bg-black/5 dark:bg-black/20">
                              {msg.messageType === 'image' ? (
                                ...
                                <div className="relative max-w-full overflow-hidden">
                                  {msg.status === 'uploading' ? (
                                    <div className={`p-3 flex flex-col justify-center gap-2 rounded-xl w-72 xs:w-80 sm:w-96 select-none ${isOwnMessage
                                      ...
                                      <button
                                        onClick={() => handleCancelUpload(msg._id, msg.fileAttachment.url)}
                                        className={`mt-1.5 px-3 py-1 self-start text-[10px] font-bold rounded-lg transition-all cursor-pointer ${isOwnMessage ? 'bg-white/20 hover:bg-white/30' : 'bg-black/10 hover:bg-black/15'
                                          }`}
                                      >
                                        Cancel
                                      </button>
                          ...
                          {(!msg.fileAttachment || msg.text) && (
                            <p className={`text-left break-words whitespace-pre-wrap select-text pb-3.5 ${isOwnMessage
                      ...
                      <div className={`absolute bottom-1 right-2.5 flex items-center gap-1 text-[9px] select-none whitespace-nowrap ${isOwnMessage ? 'text-white/70' : 'text-gray-400 dark:text-gray-500'
                        }`}>
                        {isMessagePinned && (
                      ...
                      {msg.reactions && msg.reactions.length > 0 && (
                        <div className={`absolute -bottom-2 flex items-center gap-1 select-none pointer-events-auto bg-white/95 dark:bg-slate-900/95 backdrop-blur-xs border border-gray-250/60 dark:border-slate-800/60 rounded-full py-0.5 px-2 shadow-sm z-20 hover:scale-105 transition-all ${isOwnMessage ? 'right-4' : 'left-4'
                          }`}>
                          {getGroupedReactions(msg.reactions, currentUser.id).map((group) => (
                            <button
                              ...
                              className={`flex items-center gap-0.5 text-[11px] font-bold px-1 rounded-full cursor-pointer hover:bg-black/5 dark:hover:bg-white/10 ${group.hasReacted ? 'text-accent dark:text-accent' : 'text-text-title'
                                }`}
                            >
                  ...
                  <div key={member.userId} className="flex items-center justify-between p-2 rounded-2xl hover:bg-bg-app/40 transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0 text-left">
                        ...
                        <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-full select-none mt-0.5 inline-block ${member.role === 'owner'
                    ...
                    contacts.filter(c =>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${isSelected ? 'bg-accent border-accent text-white shadow-sm' : 'border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800'
                            }`}>
                            {isSelected && (
                  ...
                  contacts.filter(c => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${isSelected ? 'bg-accent border-accent text-white shadow-sm' : 'border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800'
                          }`}>
                          {isSelected && (
              ...
              {['👍', '❤️', '😂', '😮', '😢', '🙏'].map((emoji) => {
                ...
                return (
                  <button
                    ...
                    className={`text-lg p-1 hover:bg-black/5 dark:hover:bg-white/10 rounded-lg cursor-pointer transition-all active:scale-125 ${hasReacted ? 'bg-accent/15 dark:bg-accent/25 border border-accent/30' : 'border border-transparent'
                      }`}
                  >
                    {emoji}
                  </button>
                ...
                {['👍', '❤️', '😂', '😮', '😢', '🙏'].map((emoji) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-2xl p-1.5 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl cursor-pointer transition-all active:scale-125 ${hasReacted ? 'bg-accent/15 dark:bg-accent/25 border border-accent/30' : 'border border-transparent'
                        }`}
                    >
                      {emoji}
                    </button>
